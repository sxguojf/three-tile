...
import {
	TileMaterialLoader as st,
	LoaderFactory as rt,
	VectorTileRender as ot,
	VectorFeatureTypes as T,
	TileSource as lt,
} from "three-tile";
function R(i, t, e, n) {
	let s = n;
...
function v(i, t, e, n) {
	...
	if (t === "Point" || t === "MultiPoint" || t === "LineString") C(s, e);
	else if (t === "Polygon") C(s, e[0]);
	else if (t === "MultiLineString") for (const r of e) C(s, r);
	else if (t === "MultiPolygon") for (const r of e) C(s, r[0]);
	return s;
}
function C(i, t) {
...
function k(i, t, e, n) {
	...
	if ((e.promoteId ? (a = t.properties[e.promoteId]) : e.generateId && (a = n || 0), r === "Point")) H(s, l);
		...
		if (e.lineMetrics) {
			for (const h of s) (l = []), j(h, l, o, !1), i.push(v(a, "LineString", l, t.properties));
			return;
		} else G(s, l, o, !1);
	else if (r === "Polygon") G(s, l, o, !0);
...
function _(i, t, e, n, s, r, o, l) {
	...
	for (const h of i) {
		...
		let x = [];
		if (u === "Point" || u === "MultiPoint") pt(c, x, e, n, s);
		else if (u === "LineString") ct(c, x, e, n, s, !1, l.lineMetrics);
		else if (u === "MultiLineString") N(c, x, e, n, s, !1);
		else if (u === "Polygon") N(c, x, e, n, s, !0);
		...
		if (x.length) {
			if (l.lineMetrics && u === "LineString") {
				for (const M of x) a.push(v(h.id, u, M, h.tags));
				continue;
			}
			(u === "LineString" || u === "MultiLineString") &&
				(x.length === 1 ? ((u = "LineString"), (x = x[0])) : (u = "MultiLineString")),
				(u === "Point" || u === "MultiPoint") && (u = x.length === 3 ? "Point" : "MultiPoint"),
				a.push(v(h.id, u, x, h.tags));
		}
	}
...
function ct(i, t, e, n, s, r, o) {
	...
	for (let p = 0; p < i.length - 3; p += 3) {
		const m = i[p],
			...
			P = s === 0 ? S : w;
		let X = !1;
		o && (c = Math.sqrt(Math.pow(m - S, 2) + Math.pow(F - w, 2))),
			V < e
				? P > e && ((u = a(l, m, F, S, w, e)), o && (l.start = h + c * u))
				: V > n
					? P < n && ((u = a(l, m, F, S, w, n)), o && (l.start = h + c * u))
					: B(l, m, F, D),
			P < e && V >= e && ((u = a(l, m, F, S, w, e)), (X = !0)),
			P > n && V <= n && ((u = a(l, m, F, S, w, n)), (X = !0)),
			!r && X && (o && (l.end = h + c * u), t.push(l), (l = $(i))),
			o && (h += c);
	}
	let d = i.length - 3;
	const f = i[d],
	...
	y >= e && y <= n && B(l, f, x, M),
		(d = l.length - 3),
		r && d >= 3 && (l[d] !== l[0] || l[d + 1] !== l[1]) && B(l, l[0], l[1], l[2]),
		l.length && t.push(l);
}
function $(i) {
	const t = [];
...
function N(i, t, e, n, s, r) {
	for (const o of i) ct(o, t, e, n, s, r, !1);
}
function B(i, t, e, n) {
	i.push(t, e, n);
}
...
function W(i, t) {
	...
	for (let n = 0; n < i.length; n++) {
		const s = i[n],
			r = s.type;
		let o;
		if (r === "Point" || r === "MultiPoint" || r === "LineString") o = O(s.geometry, t);
		else if (r === "MultiLineString" || r === "Polygon") {
			o = [];
			for (const l of s.geometry) o.push(O(l, t));
		} else if (r === "MultiPolygon") {
			o = [];
			for (const l of s.geometry) {
				const a = [];
				for (const h of l) a.push(O(h, t));
				o.push(a);
			}
		}
...
function K(i, t) {
	...
	for (const r of i.features) {
		const o = r.geometry,
			l = r.type;
		if (((r.geometry = []), l === 1))
			for (let a = 0; a < o.length; a += 2) r.geometry.push(Q(o[a], o[a + 1], t, e, n, s));
		else
			for (let a = 0; a < o.length; a++) {
				const h = [];
				for (let c = 0; c < o[a].length; c += 2) h.push(Q(o[a][c], o[a][c + 1], t, e, n, s));
				r.geometry.push(h);
			}
	}
...
function Pt(i, t, e, n) {
	...
	if (
		...
		for (let l = 0; l < s.length; l += 3) o.push(s[l], s[l + 1]), i.numPoints++, i.numSimplified++;
	else if (r === "LineString") A(o, s, i, e, !1, !1);
	else if (r === "MultiLineString" || r === "Polygon")
		for (let l = 0; l < s.length; l++) A(o, s[l], i, e, r === "Polygon", l === 0);
	else if (r === "MultiPolygon")
		for (let l = 0; l < s.length; l++) {
			const a = s[l];
			for (let h = 0; h < a.length; h++) A(o, a[h], i, e, !0, h === 0);
		}
	if (o.length) {
		let l = t.tags || null;
	...
	splitTile(t, e, n, s, r, o, l) {
		...
		for (; a.length; ) {
			...
			let m = null,
				...
				D = null,
				S = null,
				w = _(t, u, n - x, n + y, 0, f.minX, f.maxX, h),
				V = _(t, u, n + M, n + p, 0, f.minX, f.maxX, h);
			(t = null),
				w &&
					((m = _(w, u, s - x, s + y, 1, f.minY, f.maxY, h)),
					(F = _(w, u, s + M, s + p, 1, f.minY, f.maxY, h)),
					(w = null)),
				V &&
					((D = _(V, u, s - x, s + y, 1, f.minY, f.maxY, h)),
					(S = _(V, u, s + M, s + p, 1, f.minY, f.maxY, h)),
					(V = null)),
				c > 1 && console.timeEnd("clipping"),
				a.push(m || [], e + 1, n * 2, s * 2),
				a.push(F || [], e + 1, n * 2, s * 2 + 1),
				a.push(D || [], e + 1, n * 2 + 1, s * 2),
				a.push(S || [], e + 1, n * 2 + 1, s * 2 + 1);
		}
	}
	getTile(t, e, n) {
		(t = +t), (e = +e), (n = +n);
...
class ie extends st {
	 ...
	 */
	async doLoad(e, n) {
		const { x: s, y: r, z: o, source: l } = n,
			a = l.userData,
			h = "style" in l ? l.style : a.style;
		return a.gv
			? this._getTileTexture(a.gv, s, r, o, h)
			: (a.loading || ((a.loading = !0), (a.gv = await this.loadJSON(e)), (a.loading = !1)),
				await (async () => {
					for (; !a.gv; ) await new Promise(c => setTimeout(c, 100));
				})(),
				console.assert(a.gv),
				this._getTileTexture(a.gv, s, r, o, h));
	}
	...
	drawTile(e, n) {
		...
		if (l) {
			...
			const a = e.features;
			for (let h = 0; h < a.length; h++) this._renderFeature(l, a[h], n);
			l.restore();
		}
	...
	_renderFeature(e, n, s = {}) {
		const r = [T.Unknown, T.Point, T.Linestring, T.Polygon][n.type],
			o = {
				geometry: [],
				properties: {},
			};
		for (let l = 0; l < n.geometry.length; l++) {
			let a;
			Array.isArray(n.geometry[l][0])
				? (a = n.geometry[l].map(h => ({ x: h[0], y: h[1] })))
				: (a = [{ x: n.geometry[l][0], y: n.geometry[l][1] }]),
				o.geometry.push(a);
		}
	...
	_getTileTexture(e, n, s, r, o) {
		if (r < (o.minLevel ?? 1) || r > (o.maxLevel ?? 20)) return new Z();
...
L.convert = function (i) {
	...
	if (Array.isArray(i)) return new L(+i[0], +i[1]);
	if (i.x !== void 0 && i.y !== void 0) return new L(+i.x, +i.y);
...
function Lt(i, t, e) {
	i === 1
		...
		: i === 2
			? Et(e, t)
			: i === 3
				? (t.type = /** @type {0 | 1 | 2 | 3} */ e.readVarint())
...
const q = 65536 * 65536,
	z = 1 / q,
	kt = 12,
	tt = typeof TextDecoder > "u" ? null : new TextDecoder("utf-8"),
	U = 0,
	b = 1,
	I = 2,
	Y = 5;
class Xt {
	...
	readVarint(t) {
		...
		return (
			...
			((s = e[this.pos++]), (n |= (s & 127) << 21), s < 128)
				? n
				: ((s = e[this.pos]), (n |= (s & 15) << 28), Gt(n, t, this))
		);
	}
	readVarint64() {
	...
	writeFixed64(t) {
		this.realloc(8),
			...
			this.dataView.setInt32(this.pos + 4, Math.floor(t * z), !0),
			(this.pos += 8);
	}
	/** @param {number} val */
	writeSFixed64(t) {
...
class se extends st {
	...
	async doLoad(e, n) {
		const s = n.source,
			...
			o = await this._loader.loadAsync(e).catch(() => new Z()),
			l = new bt(new Xt(o)),
			a = this.drawTile(l, r, n.z);
		return new nt(a);
	}
	/**
	 * 在离屏画布上绘制矢量瓦片
		...
		if (a) {
			if ((a.scale(1, -1), a.translate(0, -256), n))
				for (const h in n.layer) {
					const c = n.layer[h];
					if (n && (s < (c.minLevel ?? 1) || s > (c.maxLevel ?? 20))) continue;
					const u = e.layers[h];
					if (u) {
	...
	_renderFeature(e, n, s = {}, r = 1) {
		const o = [T.Unknown, T.Point, T.Linestring, T.Polygon][n.type],
			l = {
...
export { ie as GeoJSONLoader, ne as GeoJSONSource, se as MVTLoader, re as MVTSource };
