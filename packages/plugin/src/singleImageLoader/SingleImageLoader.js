...
import { LoaderFactory, TileMaterial } from "three-tile";
/**
 * Single image Material loader
 */
export class SingleImageLoader {
    info = {
        version: "0.10.0",
    ...
    _imageLoader = new ImageLoader(LoaderFactory.manager);
    /**
     * 加载材质
     * @param source 数据源
     * @param tile 瓦片
    ...
    _setTexture(material, image, source, tileBounds) {
        const texture = this._getTileTexture(image, source, tileBounds);
        material.setTexture(texture);
        texture.needsUpdate = true;
    }
    _getTileTexture(image, source, tileBounds) {
