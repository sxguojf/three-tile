 ...
 * @returns sub image
 */
export function getSubImage(image, bounds) {
...
function getMaxLevelTileAndBounds(x, y, z, maxLevel) {
    ...
    const bounds = [box.min.x + 0.5, box.min.y + 0.5, box.max.x + 0.5, box.max.y + 0.5];
 ...
 */
export function tileBoundsClip(image, mapBounds, tileBounds) {
    // 瓦片完全在地图范围内，直接返回原始图像
    if (mapBounds[0] <= tileBounds[0] &&
