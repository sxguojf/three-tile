...
export function parse(data, z, clipBounds) {
    let demData = data;
    // 地形从父瓦片取需要剪裁
...
function getSubDEM(demData, bounds) {
    // 数组剪裁并缩放
    function arrayclipAndResize(buffer, width, sx, sy, sw, sh, dw, dh) {
        // clip
    ...
    // 瓦片剪裁并缩放
    const demArray = arrayclipAndResize(demData.array, demData.width, piexlRect.sx, piexlRect.sy, piexlRect.sw, piexlRect.sh, width, height);
    return { array: demArray, width, height };
}
function getBoundsCoord(clipBounds, targetSize) {
    // left-top coordinate
