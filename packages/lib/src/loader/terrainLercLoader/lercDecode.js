		...
		readDataOneSweep: function (input, data, OutPixelTypeArray, useBSQForOutputDim) {
			...
			if (rawData.length === numPixels * numDims) {
				if (useBSQForOutputDim) {
					data.pixels.resultPixels = Lerc2Helpers.swapDimensionOrder(
						rawData,
						numPixels,
						numDims,
						OutPixelTypeArray,
					...
					for (iDim = 0; iDim < numDims; iDim++) {
						...
						if (blockEncoding > 3) {
							...
							arrayBuf = new ArrayBuffer(
								numBytes % dataTypeSize === 0 ? numBytes : numBytes + dataTypeSize - (numBytes % dataTypeSize)
							);
							store8 = new Uint8Array(arrayBuf);
							...
							if (blockEncoding === 3) {
								...
								if (mask) {
									for (row = 0; row < thisBlockHeight; row++) {
										for (col = 0; col < thisBlockWidth; col++) {
											if (mask[outPtr]) {
												resultPixels[outPtr] = isDiffEncoding
													? Math.min(zMax, resultPixelsPrevDim[outPtr] + offset)
													: offset;
											}
											outPtr++;
										}
		...
		isValidPixelValue: function (t, val) {
			...
			switch (t) {
				...
				case 6:
					...
					break;
				case 7:
					isValid = val >= -1.7976931348623157e308 && val <= 1.7976931348623157e308;
					break;
				default:
					isValid = false;
			}
