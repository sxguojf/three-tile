 ...
 *@date: 2023-04-05
 */
import { VectorFeatureTypes } from "./IVectorTileRender";
/**
 * 矢量数据渲染器
 */
export class VectorTileRender {
     ...
     * @param scale 拉伸倍数
     */
    render(ctx, type, feature, style, scale = 1) {
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        // 设置阴影效果
    ...
    // 渲染点要素
    _renderPointText(ctx, feature, scale = 1, textFiled = "name", fontOffset = [0, 0]) {
        ...
        if (properties && properties[textFiled]) {
            ctx.fillText(properties[textFiled], points[0][0].x * scale + fontOffset[0], points[0][0].y * scale + fontOffset[1]);
        }
    }
    // 渲染线要素
    _renderLineString(ctx, feature, scale) {
    ...
    // 渲染面要素
    _renderPolygon(ctx, feature, scale) {
