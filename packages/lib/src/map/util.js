 ...
 */
export function getLocalInfoFromRay(map, ray) {
    const intersects = ray.intersectObjects([map.rootTile]);
 ...
 * @returns ground info
 */
export function getLocalInfoFromWorld(map, worldPosition) {
 ...
 * @returns ground info
 */
export function getLocalInfoFromScreen(camera, map, pointer) {
    const ray = new Raycaster();
    ray.setFromCamera(pointer, camera);
    return getLocalInfoFromRay(map, ray);
}
export function attachEvent(map) {
    const loadingManager = map.loader.manager;
    const dispatchLoadingEvent = (type, payload) => {
...
// 	});
// }
function drawBillboards(txt, size = 128) {
...
export function createBillboards(txt, size = 128) {
