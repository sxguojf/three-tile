...
| enableUnderground | boolean  | 默认为false，指定摄像机是否能穿过地面进入地下。              |
...
| LODThreshold      | number   | 默认为 1，设置瓦片 LOD 阈值。TileMap 本质上是一个动态 LOD 模型，它根据瓦片离摄像机的距离对模型进行细化或合并，该值越大瓦片细化越快，越小瓦片合并越快 |
| attributions      | string[] | 取得瓦片数据归属者信息，如版权等，来自瓦片数据源             |
